...
class EncodeToRawXML extends AbstractHandler
{
    ...
    public function transform(string $segment): string
    {
        ...
        $segment = preg_replace('/&(#10;|#x0A;)|\n/', '##_ent_0A_##', $segment) ?? $segment;

        // handling &#13; (carriage return)
        // prevent to convert it to \r
        $segment = preg_replace('/&(#13;|#x0D;)|\r/', '##_ent_0D_##', $segment) ?? $segment;

        // handling &#09; (tab)
        // prevent to convert it to \t
        $segment = preg_replace('/&#09;|\t/', '##_ent_09_##', $segment) ?? $segment;

        //Substitute 4(+)-byte characters from a UTF-8 string to htmlentities
        $segment = preg_replace_callback('/([\xF0-\xF7]...)/s', [Utils::class, 'htmlentitiesFromUnicode'], $segment) ?? $segment;
        ...
        return preg_replace('/&(?!lt;|gt;|amp;|quot;|apos;|#x?[0-9A-F]{1,7};)/', '&amp;', $segment) ?? $segment;
    }
}
