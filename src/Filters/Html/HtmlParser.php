...
class HtmlParser
{
    ...
    public function registerCallbacksHandler(AbstractHandler $class): void
    {
        ...
        $traits = [];
        $current = get_class($class); // returns string, safe starting point
     ...
     */
    private function handlePlainTextState(
        string $char,
        int &$state,
        string &$html_buffer,
        string &$plain_text_buffer,
        string &$output
    ): void {
        switch ($char) {
            case '<':
     ...
     */
    private function handleHtmlState(
        string $char,
        int $idx,
        int $charCount,
        int &$state,
     ...
     * Handles the '>' character in the HTML state.
     */
    private function onGreaterThanInHtml(
        string $char,
        int &$state,
        string &$html_buffer,
        string &$output,
     ...
     */
    private function onDefaultCharInHtml(
        string $char,
        int $idx,
        int $charCount,
        int &$state,
    ...
    private function handleCommentState(string $char, int &$state, string &$html_buffer, string &$output): void
    {
        ...
        if ($char === '>' && str_ends_with($html_buffer, '-->')) {
            $state = static::STATE_PLAINTEXT;
